...
def getSingleCatStats(cat, tallies, parsedData, allSort=False):
	catStats= []
	for x in tallies['members']:
		if tallies['members'][x]['all'][cat] > 0:
			tmp= []
			tmp.append(parsedData['members'][x])
			tmp.append(tallies['members'][x]['recent'][cat])
			tmp.append(tallies['members'][x]['all'][cat])
			catStats.append(tmp)

	if allSort: catStats.sort(key= lambda x: x[2], reverse=True)
	else: catStats.sort(key= lambda x: x[1], reverse=True)

	catStats= [[str(x) for x in catStats[i]] for i in range(len(catStats))]
